...
object config {
	...
	method configurarTeclas() {
		keyboard.left().onPressDo({ pepita.irA(pepita.position().left(1)) }) //le pide la posicion a pepita y mueve 1 a la izq
	...
	method cambiarSeguidor() {
		...
		const seguidorSiguiente = seguidorActual.siguiente() //variables viven y mueren en este metodo
